...
and `maxAttempts` carry the message context when known. Every such error is
also logged at error level (with the cause's stack trace when there is one),
whether or not a callback is supplied. Exceptions thrown by the callback are
caught and logged, so they never interrupt message processing.
